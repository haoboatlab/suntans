...
void InterpolateSubgridDepth(gridT *grid, physT *phys, propT *prop, int myproc,int numprocs)
{
  int i, j, jstr, n, nc,nc1,nc2,ne, Nd, scaledepth,ncount,base,base1,k,intdd,ntotal;
...
void SubgridCheck(gridT *grid, int myproc)
{
  ...
  for(i=0;i<grid->Nc;i++)
  {
    if(fabs(grid->dv[i]+subgrid->hmin[i])>1e-5){
      ...
      printf("proc %d i %d xv %e yv %e dv %e hmin %e dv+hmin %e\n",myproc,i,grid->xv[i],grid->yv[i],grid->dv[i],subgrid->hmin[i],grid->dv[i]+subgrid->hmin[i]);
    }
  }
...
void SubgridBasic(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  OpenSubgridFiles(prop->mergeArrays,myproc);
...
void SubgridFluxCheck(gridT *grid, physT *phys, propT *prop,int myproc){
  ...
  REAL sum0,normal,sum,fac1,fac2,flux;
  ...
  for(iptr=grid->celldist[0];iptr<grid->celldist[2];iptr++) {
    ...
    if(ktop!=grid->Nk[i]-1)
    {
      flux=prop->dt*(fac1*phys->wnew[i][ktop+1]*subgrid->Acveff[i][ktop+1]+
        fac2*phys->wtmp2[i][ktop+1]*subgrid->Acveffold[i][ktop+1]);
