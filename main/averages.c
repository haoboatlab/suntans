...
void AllocateAverageVariables(gridT *grid, averageT **average, propT *prop)
{
  ...
  *average = (averageT *)SunMalloc(sizeof(averageT),"AllocateAverageVariables");
  ...
  (*average)->q = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
  (*average)->counter = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables"); 
  if(prop->calcreynolds){
    (*average)->upup = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
    (*average)->vpvp = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
    (*average)->wpwp = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
    (*average)->upvp = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
    (*average)->upwp = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
    (*average)->vpwp = (REAL **)SunMalloc(Nc*sizeof(REAL *),"AllocateAverageVariables");
  }
  ...
  for(i=0;i<Nc;i++) {
    ...
    (*average)->q[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
  ...
  if(prop->calcreynolds){
    for(i=0;i<Nc;i++) {
      (*average)->upup[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
      (*average)->vpvp[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
      (*average)->wpwp[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
      (*average)->upvp[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
      (*average)->upwp[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
      (*average)->vpwp[i] = (REAL *)SunMalloc(grid->Nk[i]*sizeof(REAL),"AllocateAverageVariables");
    }
  }
...
void UpdateAverageVariables(gridT *grid, averageT *average, physT *phys, metT *met, propT *prop,MPI_Comm comm, int myproc){
    ...
    REAL z, depth_face, wbar;

  for(iptr=grid->celldist[0];iptr<grid->celldist[1];iptr++) {
    i = grid->cellp[iptr];
    average->w[i][grid->Nk[i]]+=phys->w[i][grid->Nk[i]];
    if(prop->calcreynolds){
      average->wpwp[i][grid->Nk[i]]+=phys->w[i][grid->Nk[i]]*phys->w[i][grid->Nk[i]];
    }
    //sdz=0;
    //Tdz=0;
    for(k=grid->ctop[i];k<grid->Nk[i];k++) {
      ...
      average->q[i][k]+=phys->q[i][k];
      average->nu_v[i][k]+=phys->nu_tv[i][k];
      average->kappa_tv[i][k]+=phys->kappa_tv[i][k];

      if(prop->calcreynolds){
        wbar = 0.5*(phys->wc[i][k]+phys->wc[i][k+1]);
        average->upup[i][k]+=phys->uc[i][k]*phys->uc[i][k];
        average->vpvp[i][k]+=phys->vc[i][k]*phys->vc[i][k];
        average->wpwp[i][k]+=wbar*wbar;
        average->upvp[i][k]+=phys->uc[i][k]*phys->vc[i][k];
        average->upwp[i][k]+=phys->uc[i][k]*wbar;
        average->vpwp[i][k]+=phys->vc[i][k]*wbar;
      }
...
void ComputeAverageVariables(gridT *grid, averageT *average, physT *phys, metT *met, int ntaverage, propT *prop){
  ...
  for(i=0;i<Nc;i++) {
    average->w[i][grid->Nk[i]]+=phys->w[i][grid->Nk[i]];
    if(average->counter[i][0]>0){
        nt2d = 1.0/(average->counter[i][0]);
    }else{
        nt2d= 1.0;
    }
    ...
    for(k=0;k<grid->Nk[i];k++) {
      ...
      if(prop->calcreynolds){
        // calc average of full value
        average->upup[i][k] *= nt3d;
        average->vpvp[i][k] *= nt3d;
        average->wpwp[i][k] *= nt3d;
        average->upvp[i][k] *= nt3d;
        average->upwp[i][k] *= nt3d;
        average->vpwp[i][k] *= nt3d;
      }
    ...
    // subtract out mean: u = \bar(u) + \tilde(u)
    // \bar(\tilde(u)\tilde(u)) = \bar(u * u) - \bar(u)\bar(u)
    if(prop->calcreynolds){ 
      average->wpwp[i][grid->Nk[i]]-=average->w[i][grid->Nk[i]]*average->w[i][grid->Nk[i]];
      for(k=0;k<grid->Nk[i];k++) {
        wbar = 0.5*(average->w[i][k]+average->w[i][k+1]);
        average->upup[i][k] -= average->uc[i][k]*average->uc[i][k];
        average->vpvp[i][k] -= average->vc[i][k]*average->vc[i][k];
        average->wpwp[i][k] -= wbar*wbar;
        average->upvp[i][k] -= average->uc[i][k]*average->vc[i][k];
        average->upwp[i][k] -= average->uc[i][k]*wbar;
        average->vpwp[i][k] -= average->vc[i][k]*wbar;
      }
    }
    // 2D cell-centred variables
    average->h_avg[i] *= nt2d;
    //average->s_dz[i] *= nt;
    //average->T_dz[i] *= nt;
    if(prop->metmodel>0){
    	average->Uwind[i] *= nt2d;
    	average->Vwind[i] *= nt2d;
    	average->Tair[i] *= nt2d;
    	average->Pair[i] *= nt2d;
    	average->rain[i] *= nt2d;
    	average->RH[i] *= nt2d;
    	average->cloud[i] *= nt2d;
    	average->Hs[i] *= nt2d*V0;
    	average->Hl[i] *= nt2d*V0;
    	average->Hlw[i] *= nt2d*V0;
    	average->Hsw[i] *= nt2d*V0;
    	average->tau_x[i] *= nt2d;
    	average->tau_y[i] *= nt2d;
    	average->EP[i] *= nt2d*V0;
    }
  }

  for(j=0;j<Ne;j++){
    for(k=0;k<grid->Nke[j];k++){
	    average->U_F[j][k] *= nt2d *V0;
	    average->s_F[j][k] *= nt2d *V0;
	    average->T_F[j][k] *= nt2d *V0;
...
void SendRecvAverages(propT *prop, gridT *grid, averageT *average, MPI_Comm comm, int myproc){
  ...
  ISendRecvCellData3D(average->q,grid,myproc,comm);
  ...
  if(prop->calcreynolds){
    ISendRecvCellData3D(average->upup,grid,myproc,comm);
    ISendRecvCellData3D(average->vpvp,grid,myproc,comm);
    ISendRecvCellData3D(average->wpwp,grid,myproc,comm);
    ISendRecvCellData3D(average->upvp,grid,myproc,comm);
    ISendRecvCellData3D(average->upwp,grid,myproc,comm);
    ISendRecvCellData3D(average->vpwp,grid,myproc,comm);
  }
